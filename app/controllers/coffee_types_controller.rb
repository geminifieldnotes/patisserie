class CoffeeTypesController < ApplicationController
  ...
  # GET /coffee_types or /coffee_types.json
  def index
    @coffee_types = CoffeeType.all
  end

  # GET /coffee_types/1 or /coffee_types/1.json
  def show
  end
  ...
  # POST /coffee_types or /coffee_types.json
  def create
    @coffee_type = CoffeeType.new(coffee_type_params)

    respond_to do |format|
      if @coffee_type.save
        format.html { redirect_to coffee_type_url(@coffee_type), notice: "Coffee type was successfully created." }
  ...
  # PATCH/PUT /coffee_types/1 or /coffee_types/1.json
  def update
    respond_to do |format|
      if @coffee_type.update(coffee_type_params)
        format.html { redirect_to coffee_type_url(@coffee_type), notice: "Coffee type was successfully updated." }
  ...
  # DELETE /coffee_types/1 or /coffee_types/1.json
  def destroy
    @coffee_type.destroy
  ...
  private
    ...
    def coffee_type_params
      params.require(:coffee_type).permit(:name, :description)
    end
end
